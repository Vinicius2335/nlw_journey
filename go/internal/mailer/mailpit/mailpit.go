...
func (mp Mailpit) SendConfirmTripEmailToTripOwner(tripId uuid.UUID) error {
	...
	msg.SetBodyString(mail.TypeTextPlain, fmt.Sprintf(
		...
		`, trip.OwnerEmail, trip.Destination, trip.StartsAt.Time.Format(time.DateOnly),
	))
	...
	if err := client.DialAndSend(msg); err != nil {
		return fmt.Errorf("mailpit: failed to set to send email SendConfirmTripEmailToTripOwner: %w", err)
	}
