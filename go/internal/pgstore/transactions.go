...
func (q *Queries) CreateTrip(ctx context.Context, pool *pgxpool.Pool, params spec.CreateTripRequest) (uuid.UUID, error) {
	...
	if err != nil {
		return uuid.UUID{}, fmt.Errorf("psgstore: failed to begin trx for CreateTrip: %w", err)
	}
	...
	if err != nil {
		return uuid.UUID{}, fmt.Errorf("psgstore: failed to insert trx for CreateTrip: %w", err)
	}
	...
	if _, err := qtx.InviteParticipantsToTrip(ctx, participants); err != nil {
		return uuid.UUID{}, fmt.Errorf("psgstore: failed to insert participants trx for CreateTrip: %w", err)
	}

	if err := tx.Commit(ctx); err != nil {
		return uuid.UUID{}, fmt.Errorf("psgstore: failed to commit trx for CreateTrip: %w", err)
	}
